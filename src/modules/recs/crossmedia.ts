...
export async function getCrossMediaReco(
  ...
  //    The cache is CROSS-USER, so a pairing generated by someone else may
  //    target a title THIS user already owns — filter it at read time (the
  //    council's library-check gap): quiet empty, never an owned reco. The
  //    seed's cache slot is taken, so there's nothing to regenerate.
...
export async function getCrossMediaFeed(userId: string): Promise<CrossMediaFeed> {
  ...
  // Cache-first read, then drop pairings whose target THIS user already owns
  // (the shared cache may hold another user's generation — same read-time
  // library check as getCrossMediaReco, batched in one ownership query).
  const cachedPairs: { seed: LovedSeed; reco: CrossMediaReco }[] = [];
  for (const seed of seeds) {
    const reco = await readCache(seed.catalogItemId);
    if (reco) cachedPairs.push({ seed, reco });
  }
          ...
          await db
            ...
            .where(
              and(
                eq(userItems.userId, userId),
                inArray(
                  userItems.catalogItemId,
                  cachedPairs.map((p) => p.reco.targetCatalogItemId),
                ),
              ),
            )
        ).map((r) => r.catalogItemId),
  );
  const items: CrossMediaFeedItem[] = cachedPairs
    .filter((p) => !ownedTargets.has(p.reco.targetCatalogItemId))
    .map((p) => toFeedItem(p.seed, p.reco));
