 ...
 * with the middle stop at 50% (own profile) or 55% (public), .65, blur 90,
 * hanging off the top edge and fading out at 72% instead of being cut off —
 * and over it the 620px overlay that lifts the top-right corner and fades
 * everything into the page background. Both are `absolute`, so the caller's
 * root must be `relative` and the content must sit in its own `relative`
 * layer above.
 */

/** The mock's page-hero fade, so the glow ends in air instead of an edge. */
const GLOW_MASK = "linear-gradient(#000 72%, transparent 100%)";
...
export function ProfileBackdrop({
  ...
  return (
    <>
      <div
        aria-hidden
        className="pointer-events-none absolute -inset-x-[60px] -top-[120px] h-[480px] opacity-65"
        style={{
