...
/**
 * AI provenance on the ficha (Revamp UI 06, redrawn in Kura 2026-09-24: no
 * accent — honey is reserved for Seguir — and no dots). The provenance is one
 * glass row in the body —
  ...
  return (
    <div className="flex flex-col gap-3">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="flex min-h-[52px] w-full items-center gap-2.5 rounded-[var(--r-surface)] bg-[var(--glass-bg)] px-4 py-3.5 text-left transition-colors active:bg-white/[0.12]"
      >
        <FillIcon d={SPARKLE_PATH} size={12} className="flex-none text-text-2" />
      ...
      {open && (
        <div className="bl-rise-soft flex flex-col gap-3 px-1">
          ...
          {narrative.closer && (
            <p className="font-serif text-[16px] italic leading-[1.45] text-pretty text-text-2">
              {narrative.closer}
            </p>
          )}
          <CrossMediaFeedback
