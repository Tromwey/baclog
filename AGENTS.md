...
- **The design system is borderless, glow-free, and pulse-free** (item-flow HANDOFF §7: "el aura es la única fuente de luz"). Do NOT add `border-*` to buttons/chips/cards/inputs/sheets (flat surface fills + fill-change selection/focus states instead), no light/colored `shadow-*` glows, no heartbeat/pulse animations on icons or glyphs. Exempt: dashed mock affordances ("Nuevo backlog" card, empty tile), content hairline dividers (coach marks, group headers, attribution, stat dividers), dark neutral depth shadows (dock, tinted cover, portaled sheets), text-shadows, loading-skeleton pulses, one-shot entrance animations, and the aura system itself. User-facing copy in the signed-in app says **backlog**, never "estante".

- **Kura is the brand; the rebrand lives ONLY on the public surfaces (2026-09-24).** `/login`, `/verify`, `/blocked`, `/waitlist`, `/creditos`, every `/u/*` page and the global metadata/manifest/icons follow `design/kura/sistema-de-diseno.dc.html`; the signed-in app (`(app)/**`) is still the Revamp UI with lima. The switch is the `.kura` class on a public page's root (`globals.css`), which re-points the accent tokens to honey — never add `.kura` inside `(app)`, and never restyle a shared primitive to Kura directly (add a variant, like `FollowButton variant="kura"`). On Kura surfaces the copy says **colección**, never backlog; honey appears once per screen and only on Seguir; there is no red. The native app is `ios/` (SwiftUI), brief in `ios/BRIEF.md`.
