...
- **Authorization is 100% app-layer** (`src/authz`), no RLS. Server actions derive the user via `assertUser`/`assertOwnsBacklog`; never accept a `userId` across an RPC boundary. `getPublicProfile` in `modules/backlog/public.ts` is the one deliberate exception (gated on `users.isPublic`, public-safe field list, identical 404 for private vs. nonexistent — no enumeration oracle).
