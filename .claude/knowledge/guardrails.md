...
| `pnpm tsx scripts/api-smoke.ts --base http://localhost:3010/api/v1 --email <cuenta> --log <log del dev server>` | manual (correr al tocar `src/authz/api.ts`, `src/auth/session.ts`, `src/app/api/v1/**` o `_lib/schemas.ts`; y antes de `pnpm ship` con cambios de API) | Que la API móvil deje pasar lo que no debe o rompa el contrato con la app sin que nadie lo vea: un bearer manipulado / con `aud` ajeno / expirado / de usuario borrado devolviendo algo que no sea el MISMO 401; un `Me` con `birthYear` o `isAdmin`; una respuesta v1 sin `private, no-store`; un error fuera del contrato `{ error: { code, message } }`; el puente `apiContext` → `getCurrentUser()` dejando de ver al usuario del bearer (`GET /me` pasa por `assertUser()` a propósito) | `scripts/api-smoke.ts` |
...
- `pnpm tsx scripts/api-smoke.ts --base <url>/api/v1 --email <cuenta> --log <archivo>` — smoke de la API v1 contra un servidor VIVO (dev local: `pnpm exec next dev -p 3010 > <log> 2>&1 &`, el OTP se lee del log); valida cada respuesta contra los zod de `src/app/api/v1/_lib/schemas.ts`; secciones `auth` · `reads` · `writes` (`--only`); `writes` solo con cuenta QA desechable (la DB es prod); sale 1 al primer fallo.
