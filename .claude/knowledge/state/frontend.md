...
| `src/app/(app)/perfil/` · `settings/` | Perfil propio (F3.10: forma de perfil público + "Tu gente" + escaparate de estantes; `edit-shelves.tsx` = hoja Privado/Público/Perfil por backlog F3.10.1; `siguiendo/`·`seguidores/`; `people-list.tsx` exporta `PersonRowView`, la fila que también renderiza Buscar gente) y Ajustes (absorbió instalar/admin/cerrar sesión tras el chip) |
