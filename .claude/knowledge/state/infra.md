...
| `public/kura-icon.svg` | Fuente ÚNICA del ícono (sistema Kura §marca · ícono: cuatro columnas de portadas, la pieza miel lleva la **k** del wordmark). La k va como `<path>` (contorno de `Newsreader-MediumItalic.ttf` sacado con fontTools), no como `<text>`: `qlmanage` no tiene Newsreader y caería a otra itálica. Se rasteriza con `qlmanage -t -s 1024` + `sips -z` a `public/icon-192/512`, `apple-touch-icon`, `src/app/favicon.ico` (ICO con PNG 16/32/48) y a iOS `AppIcon-1024.png` (×2, aplanado a RGB sin alfa) + `KuraMark.imageset` — todos salen del mismo SVG. `scripts/generate-icons.mjs` quedó obsoleto (dibujaba el destello lima) |
