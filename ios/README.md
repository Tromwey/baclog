...
- **Inicio de sesión y fusionar cuentas** (fase 4g, Ajustes › "inicio de sesión", `Features/Settings/AccountLinkViews.swift`): la fila Correo muestra el correo de la cuenta (no se toca). Apple y Google solo salen si `GET me/identities` los lista (Google además necesita el client id de `auth/providers`). **Conectar** abre la hoja del proveedor (Apple por código con `AppleAuthorization`; Google con el mismo `GoogleOAuth`) y manda `POST me/identities/{provider}`. **Conectada** tiene Desconectar, con hoja de confirmación → `DELETE me/identities/{provider}`. Si el servidor responde `409 linked_elsewhere`, trae `error.mergeToken` + `error.source` dentro del envelope, y la app va directo a la confirmación de fusión. **Fusionar otra cuenta** (`Route.mergeAccount`) ofrece Apple, Google o el correo de la otra cuenta + un código de 6 dígitos (`me/merge/otp/request` → `Route.mergeCode` → `me/merge/otp/verify`). Todo termina en `Route.mergeConfirm`: la cuenta origen con sus conteos, lo que pasa y el botón "Fusionar cuentas" → `POST me/merge`. Al terminar, el `Me` devuelto se aplica, la biblioteca se vuelve a pedir (bootstrap), feed/gente/recap quedan marcados como viejos, la app regresa a Ajustes y sale un aviso. Cuando se rechaza la prueba (el token del proveedor o el código), el servidor responde `422 invalid` con `reason: "invalid_proof"`, que la app lee como `forbidden("proof_rejected")`; `401` queda solo para una sesión muerta. `me/identities` trae `emailIsRelay`. Si es true y Apple es la única conexión, la fila de Apple explica que es la única entrada y no ofrece Desconectar; el `409 last_way_in` de `DELETE` da la misma explicación en un aviso. Si la cuenta origen era privada (`source.isPublic = false`) y la tuya es pública, la confirmación dice que sus colecciones siguen privadas pero sus reseñas, completados y obsesiones se verán en tu perfil. El `429` de `merge/otp/request` (60 s o tope de 3 códigos por hora por correo) se muestra en minutos, y "Mandar otro código" queda apagado con cuenta regresiva hasta `retryAfterSeconds`. `409 merge_token_invalid` → "Pasaron más de 10 minutos…" y regresa a la pantalla para elegir; `403 underage` → aviso y regresa a Ajustes.
...
| 11 · inicio de sesión (4g) | `identities` (Ajustes con Apple por conectar y Google conectada; tocar Conectar en Apple → 409 → confirmación) · `unlinkidentity` · `mergechooser` · `mergecode` (en el mock, `000000` se rechaza) · `mergeconfirm` (origen privado: sale el aviso de visibilidad; `-kuraMergeError underage\|expired\|offline` hace fallar la fusión) · `lastwayin` (correo relay, Apple única conexión) · `lastwayintoast` (el 409 `last_way_in`) · `mergelimit` (429 de 40 min; `-kuraMergeLimit YES` en cualquier pantalla) |
